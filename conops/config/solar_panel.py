...
from ..common.vector import boresight_axis_permutation, vecnorm
...
class SolarPanel(ConfigModel):
    ...
    def panel_illumination_fraction(
        ...
        Args:
            ...
            boresight_axis: Spacecraft boresight axis in user body frame (e.g. "+X", "+Z")
        ...
        # scbodyvector() returns the sun vector in the internal +X-boresight frame.
        # Convert panel normal from user body frame to internal frame so the dot
        # product is consistent: n_internal = P^T @ n_user.
        normal = np.array(self.normal, dtype=np.float64)
        if boresight_axis != "+X":
            p_mat = boresight_axis_permutation(boresight_axis)
            normal = p_mat.T @ normal
        ...
        for idx, time_idx in enumerate(indices):
            ...
            # Convert sun vector to internal body frame
...
class SolarPanelSet(ConfigModel):
    ...
    def illumination_and_power(
        ...
        if isinstance(time, (int, float)):
            ...
            return self._illumination_and_power_loop(
                time, ra, dec, ephem, boresight_axis=boresight_axis
            )
        ...
        # Get sun vector in internal body frame (+X boresight)
        ...
        # Convert panel normals from user body frame to internal frame.
        # geom.normal is shape (P, 3) with rows = user-frame normals.
        # N_internal = N_user @ P  (equivalent to applying P^T to each row normal).
        normals = geom.normal
        if boresight_axis != "+X":
            p_mat = boresight_axis_permutation(boresight_axis)
            normals = normals @ p_mat

        # Vectorized panel illumination calculation: dot product of normal with sun direction
        # illum per panel: shape (P,)
        panel_illum = np.dot(normals, sun_normalized)
    ...
    def optimal_charging_pointing(
        self, time: float, ephem: rust_ephem.Ephemeris, boresight_axis: str = "+X"
    ) -> tuple[float, float]:
        ...
        # Compute weighted average normal vector in user body frame
        avg_normal = np.zeros(3)
        for p in panels:
            weight = p.max_power / total_power
            avg_normal += np.array(p.normal) * weight

        # Convert to internal frame so dominant-axis logic applies consistently.
        # In the internal frame, axis 0=boresight(+X), 1=+Y(side), 2=+Z.
        if boresight_axis != "+X":
            p_mat = boresight_axis_permutation(boresight_axis)
            avg_normal = p_mat.T @ avg_normal

        # Determine dominant axis in internal frame
        abs_normal = np.abs(avg_normal)
        dominant_axis = np.argmax(abs_normal)

        if dominant_axis == 1:  # Y is dominant in internal frame (side-mounted-like)
