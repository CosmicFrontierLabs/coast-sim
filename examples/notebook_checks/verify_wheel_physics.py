...
print('Wheel params: max_torque=', wheel.max_torque, 'max_momentum=', wheel.max_momentum)
...
print('t\taccel_req(deg/s2)\treq_torque(Nm)\tallowed_calc(Nm)\tdelta_exp(Nm*s)\tdelta_act(Nm*s)\tcurr_mom(Nm*s)')
while t <= end_t + 1e-12:
    ...
    print(f"{t:.2f}\t{accel_req:.4f}\t{req_torque:.6f}\t{allowed_calc:.6f}\t{expected_delta:.6f}\t{actual_delta:.6f}\t{curr_m:.6f}")
    # compare
