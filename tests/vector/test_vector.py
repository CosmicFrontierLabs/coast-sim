...
class TestScbodyvector:
    def test_scbodyvector_zero_angles(self, ecivec_x: np.ndarray) -> None:
        """Test spacecraft body vector with zero angles."""
        result = scbodyvector(0, 0, 0, ecivec_x)
        assert result.shape == (3,)

    def test_scbodyvector_with_roll(self, ecivec_y: np.ndarray) -> None:
    ...
    def test_scbodyvector_various_angles(self, ecivec_xyz: np.ndarray) -> None:
...
class TestRotvec:
    def test_rotvec_axis1(self, x_axis: np.ndarray) -> None:
        """Test rotation around axis 1."""
        result = rotvec(1, np.pi / 2, x_axis)
        np.testing.assert_array_almost_equal(result, [1, 0, 0])

    def test_rotvec_axis2(self, x_axis: np.ndarray) -> None:
        """Test rotation around axis 2."""
        result = rotvec(2, np.pi / 2, x_axis)
        np.testing.assert_array_almost_equal(result, [0, 0, 1])

    def test_rotvec_axis3(self, x_axis: np.ndarray) -> None:
        """Test rotation around axis 3."""
        result = rotvec(3, np.pi / 2, x_axis)
        np.testing.assert_array_almost_equal(result, [0, -1, 0])

    def test_rotvec_full_rotation(self, test_vector: np.ndarray) -> None:
...
class TestGreatCircle:
    def test_great_circle_same_point(self, small_npts: int) -> None:
    ...
    def test_great_circle_different_points(self, large_npts: int) -> None:
    ...
    def test_great_circle_varying_npts(self, small_npts: int) -> None:
...
class TestRollOverAngle:
    def test_roll_over_angle_no_rollover(self, no_rollover_angles: list[float]) -> None:
    ...
    def test_roll_over_angle_positive_rollover(
        self, positive_rollover_angles: list[float]
    ) -> None:
    ...
    def test_roll_over_angle_negative_rollover(
        self, negative_rollover_angles: list[float]
    ) -> None:
    ...
    def test_roll_over_angle_multiple_rollovers(
        self, multiple_rollover_angles: list[float]
    ) -> None:
    ...
    def test_roll_over_angle_single_value(self, single_angle: list[float]) -> None:
    ...
    def test_roll_over_angle_large_jump_near_poles(self) -> None:
